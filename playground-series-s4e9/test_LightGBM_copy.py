...
def lgb_evaluate(X_train, y_train, num_leaves, max_depth, learning_rate, n_estimators, subsample, colsample_bytree, min_child_weight):
    """Evaluate LightGBM model with given hyperparameters."""
    params = {
...
def main():
    ...
    logging.info("Starting Bayesian Optimization for LightGBM...")
    lgb_bo = BayesianOptimization(
        lambda num_leaves, max_depth, learning_rate, n_estimators, subsample, colsample_bytree, min_child_weight: lgb_evaluate(
            X_train, y_train, num_leaves, max_depth, learning_rate, n_estimators, subsample, colsample_bytree, min_child_weight
        ),
        {
            'num_leaves': (20, 50),
            'max_depth': (3, 10),
    ...
    best_lgb_rmse = root_mean_squared_error(y_test, best_lgb_pred)
    logging.info(f"Best LightGBM RMSE with Bayesian Optimization: {best_lgb_rmse}")

    # Predict on test data
    logging.info("Making predictions on test data...")
    final_pred = best_lgb.predict(Scale_test)
    df_sub['price'] = np.expm1(final_pred)  # Reverse the log transformation
    submission_file = f'submission_{best_lgb_rmse:.4f}.csv'
